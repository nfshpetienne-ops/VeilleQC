...
function catClass(category)  { return CAT_META[category]?.cls  || 'cat-android'; }
function dotClass(category)  { return CAT_META[category]?.dot  || 'dot-android'; }
function catEmoji(category)  { return CAT_META[category]?.emoji || '📰'; }
...
function renderArticleCard(article, searchQuery = '') {
  ...
  const thumb = article.thumbnail
    ? `<img class="article-thumbnail" src="${esc(article.thumbnail)}"
             alt="" loading="lazy"
             onerror="this.parentNode.replaceChild(makePlaceholderThumb('${catEmoji(article.category)}'), this)">`
    : `<div class="article-thumbnail placeholder">${catEmoji(article.category)}</div>`;

  return `
    <div class="article-card${readCls}"
         data-id="${esc(article.id)}"
         data-url="${esc(article.url)}">
      ...
      <div class="article-body">
        ...
        <div class="article-meta">
          <span class="article-time">${relativeTime(article.publishedAt)}</span>
          <span>·</span>
      ...
      <div class="article-actions">
        <button class="action-btn bookmark-btn${bookmarkCls}"
                ...
                onclick="event.stopPropagation(); App.toggleBookmark('${esc(article.id)}')">
  ...
  return Array.from({ length: count }, () => `
    <div class="skeleton-card">
      ...
      <div style="flex:1;display:flex;flex-direction:column;gap:6px">
        ...
        <div class="skeleton" style="width:60%;height:10px;border-radius:3px"></div>
        <div style="display:flex;gap:6px">
...
function renderEmptyState(type = 'empty') {
  const configs = {
    empty: {
      icon: `<svg width="48" height="48" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
               <path d="M19 11H5M19 11a7 7 0 01-7 7M19 11a7 7 0 00-7-7M5 11a7 7 0 007 7M5 11a7 7 0 017-7"/>
             </svg>`,
      title: 'Aucun article',
      desc: 'Lance un rafraîchissement pour récupérer les derniers articles.',
      action: 'Rafraîchir maintenant',
      onclick: 'App.refresh()',
    },
    no_results: {
      icon: `<svg width="48" height="48" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
      ...
      desc: 'Essaie d\'autres mots-clés ou modifie tes filtres.',
  ...
  return `
    <div class="list-state">
      ...
      <p>${cfg.desc}</p>
      <button class="state-action" onclick="${cfg.onclick}">${cfg.action}</button>
    </div>`;
}
...
function renderSidebarCategories(categories, activeCategory, unreadCounts) {
  ...
  const allItem = `
    <div class="sidebar-item ${activeCategory === null ? 'active' : ''}"
         onclick="App.setCategory(null)">
